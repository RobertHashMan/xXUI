...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/RobertHashMan/xXUI/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://github.com/RobertHashMan/xXUI/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz"
